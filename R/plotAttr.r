...
#' Calculate Plot and Species-Level Attributes
#'
#' Calculates plot/stand attributes for each species and for all species
#' combined. Depending on region, additional attributes are calculated. See script (plotAttr.R) for more detailed documentation.
#'
#' @param data Data frame containing tree records from a single stand or plot.
#'   Must contain stand/plot ID, DBH, height, species, expansion factor, and
#'   crown width columns.
#' @param region Integer USFS region number (1, 2, 3, 8, or MPSG (Mountain Planning Services Group, R1-4)).
#' @param stand Character name of stand/plot ID column. Default is "StandID".
#' @param species Character name of species column (USDA plant symbol).
#'   Default is "SpeciesPLANTS".
#' @param dbh Character name of DBH column. Default is "DBH".
#' @param ht Character name of total height column. Default is "Ht".
#' @param crwidth Character name of crown width column. Default is "CrWidth".
#' @param expf Character name of expansion factor (TPA) column. Default is "TPA".
#' @param vol1 Character name of column pertaining to total cubic foot (wetern variants)
#'    or merchantable cubic foot volume (eastern variants) of tree records. Default is "MCuFt".
#'    Currently vol1 argument is only used if region argument is set to 8.
#' @param vol2 Character name of column pertaining to merchantable cubic foot (western variants) or sawlog cubic foot
#'    volume (eastern variants) of tree records in data argument. Default is "SCuFt". Currently vol2 is only
#'    used if region argument is set to 8.
#' @param vol3 Character name of column pertaining to
#'    board foot (western variants) or sawlog board foot volume (eastern
#'    variants) of tree records in data argument. Default is "SBdFt". Currently vol3 is only used if region
#'    argument is set to 8.
#' @param min Minimum DBH to include in calculations. Default is 0.
#' @param max Maximum DBH to include in calculations. Default is 999.
#' @param debug Logical; if `TRUE`, prints debug output. Default is `FALSE`.
#'
#'
#' @return Named list containing one element per species plus `ALL`. Each list element is a named numeric vector with computed plot attributes:
#'   \item{N}{Number of records in plot/stand}
#'   \item{BA}{Basal Area}
#'   \item{TPA}{Trees per acre}
#'   \item{QMD}{Quadratic mean diameter}
#'   \item{UNCC}{Percent canopy cover uncorrected for overlap}
#'   \item{CC}{Percent canopy cover corrected for overlap}
#'   \item{ZSDI}{Zeide Stand Density Index}
#'   \item{RSDI}{Reineke Stand Density Index}
#'   \item{BA_WT_DIA}{Basal area weighted diameter}
#'   \item{BA_WT_HT}{Basal area weighted height}
#'
#' For Region 1, average height (`AVE_HT`) is also calculated.
#'
#' For Regions 8 and 9, additional size-class attributes are calculated:
#'    \item{SSSIZE}{TPA weighted average height of advanced regeneration size class}
#'    \item{SSTPA}{Trees per acre of advanced regeneration size class}
#'    \item{SSBA}{Basal area of advanced regeneration size class}
#'    \item{NMSIZE}{Basal area weighted diameter of non merchantable size class}
#'    \item{NMBA}{Basal area of non merchantable size class}
#'    \item{PWSIZE}{Basal area weighted diameter of pulpwood size class}
#'    \item{PWBA}{Basal area of pulpwood size class}
#'    \item{STSIZE}{Basal area weighted diameter of saw timber size class}
#'    \item{STBA}{Basal area of saw timber size class}
#'
#' @examples
#' df <- data.frame(
#'   StandID = c("S1", "S1"),
#'   SpeciesPLANTS = c("PIPO", "ABGR"),
#'   DBH = c(10, 14),
#'   Ht = c(45, 55),
#'   CrWidth = c(12, 14),
#'   TPA = c(60, 40),
                     ...
                     debug = F)
{
  ...
  for(i in 1:nrow(data))
  {
    ...
    if(data[[dbh]][i] >= min & data[[dbh]][i] < max)
    {
      ...
      if(is.numeric(region) && region %in% c(8,9))
      {
...
#' Correct Percent Canopy Cover for Overlap
#'
#' Converts uncorrected percent canopy cover values to corrected canopy cover
#' using the Crookston and Stage (1999) relationship.
#'
#' @param CC Numeric uncorrected percent canopy cover value.
#'
#' @return Numeric corrected percent canopy cover value.
#'
#' @examples
#' correctCC(35)
#' @export
...
#pools found in FVS_TreeList or FVS_TreeList.
#
#Arguments:
#
...
#' Calculate Stand Volume Metrics
#'
#' Calculates six stand-level volume metrics from tree records. By default, the
#' function computes three live volume pools and three mortality volume pools
#' using DBH thresholds.
#'
#' @param data Data frame containing tree records from a single stand or plot.
#'   Must contain stand/plot ID, DBH, expansion factors, and volume columns.
#' @param stand Character name of stand/plot ID column. Default is "StandID".
#' @param dbh Character name of DBH column. Default is "DBH".
#' @param expf Character name of live-tree expansion factor column. Default is "TPA".
#' @param expfM Character name of mortality expansion factor column.
#'   Default is "MortPA".
#' @param vol1 Character name of first volume column. Default is "TCuFt".
#' @param vol2 Character name of second volume column. Default is "MCuFt".
#' @param vol3 Character name of third volume column. Default is "BdFt".
#' @param vol1DBH Minimum DBH included in `VOL1` and `VOL4` calculations.
#'   Default is 0.
#' @param vol2DBH Minimum DBH included in `VOL2` and `VOL5` calculations.
#'   Default is 5.
#' @param vol3DBH Minimum DBH included in `VOL3` and `VOL6` calculations.
#'   Default is 9.
#' @param debug Logical; if `TRUE`, prints debug output. Default is `FALSE`.
#'
#' @return Named numeric vector with:
#'   \item{VOL1}{Live volume pool 1}
#'   \item{VOL2}{Live volume pool 2}
#'   \item{VOL3}{Live volume pool 3}
#'   \item{VOL4}{Mortality volume pool 1}
#'   \item{VOL5}{Mortality volume pool 2}
#'   \item{VOL6}{Mortality volume pool 3}
#'
#' @examples
#' df <- data.frame(
#'   StandID = c("S1", "S1"),
