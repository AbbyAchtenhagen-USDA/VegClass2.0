...
################################################################################
##' Calculate MPSG Standardized Vegetation Variables
#'
#' Calculates standardized vegetation classification and structural variables for use by the Mountain Planning Services Group (MPSG).
#'
#' @param data Data frame containing tree records from a single stand or plot. Must include columns for stand/plot ID, DBH, species code (USDA plant symbol), expansion factor, and height.
#' @param stand Name of the column for stand or plot ID in data. Default is "StandID".
#' @param species Name of the column for USDA plant symbols in data. Default is "SpeciesPLANTS".
#' @param dbh Name of the column for DBH in data. Default is "DBH".
#' @param expf Name of the column for TPA (expansion factor) in data. Default is "TPA".
#' @param ht Name of the column for tree height in data. Default is "Ht".
#' @param crwidth Name of the column for crown width in data. Default is "CrWidth".
#' @param TPA Trees per acre for the stand/plot.
#' @param BA Basal area for the stand/plot.
#' @param CC Percent canopy cover for the stand/plot.
#' @param plotvals Named list of plot attributes for each species and/or all species in the plot/stand.
#' @param InvDB Connection to inventory database.
#' @param InvStandTbl Name of the stand data table in InvDB.
#' @param MPSGcovTyp Integer for USFS region number whose algorithms are used for COVERTYPE_MPSG calculation.
#' @param addHSS Logical; if TRUE, includes USFS Region 2 Habitat Structural Stage (HSS) variables in output.
#' @param debug Logical; if TRUE, prints debug output to R console.
#'
#' @return A named list containing:
#'   \item{COVERTYPE_MPSG}{MPSG cover type}
#'   \item{TREE_SIZE_CLASS}{MPSG tree size class}
#'   \item{CROWN_CLASS}{MPSG percent canopy cover class}
#'   \item{VERTICAL_STRUCTURE}{Number of canopy layers}
#'   \item{HSS1_4C}{Habitat structural stage (1 through 4C classes), if addHSS = TRUE}
#'   \item{HSS1_5}{Habitat structural stage (1 through 5 classes), if addHSS = TRUE}
#'
#' @examples
#' df <- data.frame(
#'   StandID = rep("S1", 5),
#'   SpeciesPLANTS = c("PIPO", "PIPO", "POTR5", "ABLA", "PICO"),
#'   DBH = c(12, 15, 8, 10, 20),
#'   TPA = c(50, 60, 40, 30, 20),
#'   Ht = c(30, 35, 25, 28, 40),
#'   CrWidth = c(10, 12, 8, 9, 15)
#' )
#' plotvals <- list()
#' MPSG(
#'   data = df,
