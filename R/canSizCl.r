...
##'
#' Calculate Canopy Size Class (canSizCl)
#'
#' Calculates canopy size class for a stand/plot in accordance with Region 3
#' rulesets from Vandendriesche, D., 2013. A Compendium of NFS Regional
#' Vegetation Classification Algorithms. USDA Forest Service. Fort Collins, CO.
#' (2013 pg.R3-3 - R3-4).
#'
#' @param data Data frame containing tree records from a single stand or plot. Must contain stand/plot ID, DBH, expansion factor, and crown width columns.
#' @param stand Character string for stand/plot ID column name. Default is "StandID".
#' @param dbh Character string for DBH column name. Default is "DBH".
#' @param expf Character string for expansion factor column name. Default is "TPA".
#' @param crwidth Character string for crown width column name. Default is "CrWidth".
#' @param TPA Trees per acre of stand/plot.
#' @param CC Percent canopy cover corrected for overlap of stand/plot.
#' @param type Indicator for diameter class type: 1 = Midscale mapping (default), 2 = Timberland dominance, 3 = Woodland dominance.
#' @param debug Logical; if TRUE, prints debug output to R console. Default is FALSE.
#'
#' @returns Integer code for canopy size class.
#' @returns Possible return values when type = 1
#'
#' @returns 1 = seedling sapling canopy cover
#' @returns 2 = small tree canopy cover
#' @returns 3 = medium and large tree canopy cover
#' @returns 4 = very large tree canopy cover
#' @returns 5 - giant tree canopy cover
#'
#' @returns Possible return values when type = 2
#
#' @returns 1 = seedling sapling canopy cover
#' @returns 2 = small tree canopy cover
#' @returns 3 = medium tree canopy cover
#' @returns 4 = large - giant tree cover
#
#' @returns Possible return values when type = 3
#
#' @returns 1 = seedling sapling canopy cover
#' @returns 2 = small tree canopy cover
#' @returns 3 = medium - giant tree cover
#'
#' @examples
#' Example usage:
#' canSizCl(data, stand = "StandID", dbh = "DBH", expf = "TPA", crwidth = "CrWidth", TPA = 300, CC = 45, type = 1)
#'
#' @export
...
##'
#' Get Canopy Size Diameter Class (getCanSizeDC)
#'
#' Returns a diameter class used to calculate canopy size class in the canSizCl
#' function.
#'
#' @param DBH DBH value.
#' @param type Indicator for diameter class type: 1 = Midscale mapping
#'   (default), 2 = Timberland dominance, 3 = Woodland dominance.
#' @param debug Logical; if TRUE, prints debug output to R console. Default is
#'   FALSE.
#'
#' @returns Integer code for diameter class (see documentation for possible
#'   values by type).
#'
#' @returns Possible return values when type = 1
#
