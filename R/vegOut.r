...
#' Calculate Stand-Level Vegetation Output Variables
#'
#' Calculates a single-row output data frame of stand-level vegetation metrics
#' and region-specific classification variables from a tree list.
#'
#' Core metrics include canopy cover, BA/TPA/QMD/SDI summaries for all trees
#' and stems, QMD of top 20 percent of TPA, and basal-area weighted diameter
#' and height. Additional outputs are appended according to the selected
#' regional rule set (`1`, `2`, `3`, `8`, or `"MPSG"`).
#'
#' @param data Data frame containing tree records for a single stand or plot.
#'   Must include stand ID, species, DBH, expansion factor, height, crown width,
#'   and volume columns used by this function.
#' @param region Region identifier used to select classification rules. Valid
#'   values are `1`, `2`, `3`, `8`, or `"MPSG"`. Default is `3`.
#' @param MPSGcovTyp Integer region code used in MPSG cover type calculations.
#'   Used when `region = "MPSG"`.
#' @param addHSS Logical; if `TRUE`, include HSS outputs for MPSG runs.
#' @param stand Character name of stand/plot ID column in `data`.
#'   Default is `"StandID"`.
#' @param species Character name of species (USDA plant symbol) column in
#'   `data`. Default is `"SpeciesPLANTS"`.
#' @param dbh Character name of DBH column in `data`. Default is `"DBH"`.
#' @param ht Character name of height column in `data`. Default is `"Ht"`.
#' @param expf Character name of expansion factor (TPA) column in `data`.
#'   Default is `"TPA"`.
#' @param crwidth Character name of crown width column in `data`.
#'   Default is `"CrWidth"`.
#' @param vol1 Character name of first volume column in `data`.
#'   Default is `"MCuFt"`.
#' @param vol2 Character name of second volume column in `data`.
#'   Default is `"SCuFt"`.
#' @param vol3 Character name of third volume column in `data`.
#'   Default is `"SBdFt"`.
#' @param vol1DBH Minimum DBH threshold used for custom volume calculations
#'   passed through to `customAttr()`.
#' @param vol2DBH Minimum DBH threshold used for custom volume calculations
#'   passed through to `customAttr()`.
#' @param vol3DBH Minimum DBH threshold used for custom volume calculations
#'   passed through to `customAttr()`.
#' @param InvDB Path to inventory database used by region-specific routines
#'   (notably Region 1 and MPSG processing).
#' @param InvStandTbl Character name of inventory stand table in `InvDB`.
#' @param customVars Optional path to custom variable specification file.
#'   If provided, custom attributes are appended to output.
#' @param debug Logical; if `TRUE`, prints debug output. Default is `FALSE`.
#'
#' @return Single-row data frame containing core stand metrics plus
#'   region-specific classification variables and optional custom attributes.
#'
#' @examples
#' \dontrun{
#' vegData <- vegOut(
                   ...
                   debug = F)
{
  ...
  if(region==2){
    ...
    #TREE_SIZE_CLASS
    vegData$TREE_SIZE_CLASS_R2<-dtResults[["TREE_SIZE_CLASS"]]

    #CROWN_COVER
    vegData$CROWN_CLASS_R2<-dtResults[["CROWN_CLASS"]]
  ...
  if (!is.null(customOutputFns) && length(customOutputFns) > 0) {
    context <- list(
      region = region,
      MPSGcovTyp = MPSGcovTyp,
      addHSS = addHSS,
    ...
    for (fn_name in unique(customOutputFns)) {
      fn <- get0(fn_name, mode = "function", inherits = TRUE)
      if (is.null(fn)) next

      extra <- tryCatch(
        fn(data = data, context = context),
        error = function(e) {
          warning(sprintf("Custom output function '%s' failed: %s", fn_name, e$message))
          NULL
        }
      )
      if (is.null(extra)) next

      if (!is.data.frame(extra)) {
        if (is.list(extra)) {
          extra <- as.data.frame(extra, stringsAsFactors = FALSE, check.names = FALSE)
        } else {
          extra <- as.data.frame(as.list(extra), stringsAsFactors = FALSE, check.names = FALSE)
        }
      }

      if (nrow(extra) != 1) {
        warning(sprintf("Custom output function '%s' returned %d rows; expected 1. Skipping.", fn_name, nrow(extra)))
        next
      }

      existing_names <- colnames(vegData)
      input_names <- colnames(extra)
      output_names <- input_names
      used_names <- existing_names
      renamed_map <- character(0)

      for (k in seq_along(input_names)) {
        base_name <- input_names[k]
        candidate <- base_name
        suffix <- 2

        while (candidate %in% used_names) {
          candidate <- paste0(base_name, suffix)
          suffix <- suffix + 1
        }

        if (!identical(candidate, base_name)) {
          renamed_map <- c(renamed_map, paste0(base_name, " -> ", candidate))
        }

        output_names[k] <- candidate
        used_names <- c(used_names, candidate)
      }

      if (length(renamed_map) > 0) {
        warning(sprintf(
          "Duplicate output column names found and renamed in custom output '%s': %s",
          fn_name,
          paste(renamed_map, collapse = ", ")
        ))
      }

      colnames(extra) <- output_names

      if (ncol(extra) > 0) {
        vegData <- cbind(vegData, extra)
      }
    }
  }
