...
################################################################################
#' Custom Attribute Calculation (customAttr)
#'
#' Provides similar functionality to the SPMCDBH() FVS Event Monitor function by returning the following attributes by species or species group, and ranges of diameters and heights:
#' - Trees per acre
#' - Basal area per acre
#' - Reineke SDI
#' - Zeide SDI
...
#' - Board foot volume/acre
#'
#' @param data Data frame containing tree records from a single stand or plot. Must contain stand/plot ID, DBH, height, species, expansion factor, and crown width columns.
#' @param region Integer variable corresponding to USFS region number (1, 2, 3, 8 or "MPSG" are valid values).
#' @param stand Character string for stand/plot ID column name. Default is "StandID".
#' @param species Character string for USDA plant symbols column name. Default is "SpeciesPLANTS".
#' @param dbh Character string for DBH column name. Default is "DBH".
#' @param ht Character string for total tree height column name. Default is "Ht".
...
#' @param vol1 Character string for total cubic foot (western) or merchantable cubic foot (eastern) volume column name. Default is "MCuFt".
#' @param vol2 Character string for merchantable cubic foot (western) or sawlog cubic foot (eastern) volume column name. Default is "SCuFt".
#' @param vol3 Character string for board foot (western) or sawlog board foot (eastern) volume column name. Default is "SBdFt".
#' @param debug Logical; if TRUE, prints debug output to R console. Default is FALSE.
#' @param custVars Dataframe specifying the arguments for the custom variables. Arguments include:
#'   - VARIABLE_NAME: name of the custom variable to be returned (required)
#'   - ATTRIBUTE: variable to be returned (required)
#'   - SPECIES: 2-letter species code or custom species group name (default=ALL)
#'   - MIN_DBH: Smallest DBH in inches to be included (default=0.0)
...
#'
##'
#' @returns Named list containing custom attributes for each plot/stand.
#'
#' @examples
#' customAttr(data, region = 3, stand = "StandID", species = "SpeciesPLANTS", dbh = "DBH", ht = "Ht", crwidth = "CrWidth", expf = "TPA")
#'
#' @export
