...
##' Calculate Density/Size Class for USFS R8
##'
#'@description
#' Calculates a density/size class based on USFS R8 rulesets defined
#' by Chad Keyser in 2023. Size density class criteria for R8 is as follows:
#'
#' If the basal area of the stand is less than 10 BA, then the size class in the
#' size density classification is advanced regeneration.
...
#' - D: 120+ BA
#'
#' @param data Data frame containing tree records for a single stand or plot. Must include a stand/plot ID column.
#' @param stand Character string for the name of the stand/plot ID column in data. Default is "StandID".
#' @param attrList List of size and density attributes for species in the stand/plot, as produced by plotAttr().
#' @param debug Logical; if TRUE, prints debug output to the R console.
#'
#' @return Character string with the density/size class code, or NA if not applicable.
#' @examples
#' attrList <- list(ALL = c(TPA=300, BA=50, NMBA=10, PWBA=20, STBA=20, ARTPA=0))
#' df <- data.frame(StandID="A", X=1)
#' denSizeR8(df, attrList=attrList)
#' @export
