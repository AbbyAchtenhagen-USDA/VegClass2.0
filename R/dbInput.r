...
#            main.R for further details.
#
#setIndices: Removing for update. Logical variable used to indicate if Case ID indices should be
...
#' Check FVS Output Database Tables (checkDBTables)
#'
#' This function performs a series of checks to see if the incoming FVS output
#' database is ready for processing. A message is returned from the function
#' which will determine if processing of input database in main (main.R)
#' function is terminated or continued.
#'
#' @param con Connection to input .db
#' @param variants Character vector of FVS variant codes associated with runs being processed in main function (main.R).
#' @param addCompute Logical. Include FVS_Compute table in output?
#' @param addPotFire Logical. Include FVS_Potfire table in output?
#' @param addFuels Logical. Include FVS_Fuels table in output?
#' @param addCarbon Logical. Include FVS_Carbon table in output?
#'
#' @return Character string containing message.
#' @examples
#' con <- RSQLite::dbConnect(RSQLite::SQLite(), ":memory:")
#' checkDBTables(con, c("CS", "LS"), TRUE, TRUE, TRUE, TRUE)
#' @export
                        ...
                        addCarbon)
{
  ...
  #=============================================================================
  #If check if FVS_TreeList is in con. If it is not, change
  #message and set validDB to FALSE.
  #=============================================================================

  if(validDB)
  {
    #Check for FVS_TreeList for any runs that assume Eastern variants
    if(is.element(T, variants %in% c("CS", "LS", "NE", "SN")))
    {
                                 ...
                                 "FVS_TREELIST"))
      {
        message<-paste("One or more runs use Eastern FVS variants (CS, LS, NE,",
                       "or SN) and FVS_TREELIST table was not found in",
                       "input database.",
                       "\n")
...
#read in data from the FVS_TreeList table depending on the
...
#
#The query will read in the following fields from the FVS_TreeList table
...
#
#variant: Two character variant code. This code is used to determine which FVS_TreeList columns are selected. Valid variant codes: "AK", "BM", "CA", "CI", "CR",
...
#' Build SQL Query for FVS TreeList Table (treeQuery)
#'
#' @description
#' Builds and returns a string of SQL statements to read data from the FVS_TreeList table, with selected columns depending on the variant specified. Can be used with dbGetQuery from RSQLite.
#'
#' The query will read in the following fields from the FVS_TreeList table when
#' variant argument is not CS, LS, NE, or SN:
#
...
#' "CrWidth", "MCuFt", "SCuFt", "SBdFt"
#'
#' @param cases Character vector of case IDs.
#' @param variant Two-character variant code. This code is used to determine which FVS_TreeList columns are selected. Valid variant codes: "AK", "BM", "CA", "CI", "CR", "CS", "EC", "EM", "IE", "KT", "LS", "NC", "NE", "PN", "SN", "SO", "TT", "UT", "WC", "WS", "OC", "OP".
#' @return Character string of SQL statements.
#' @examples
#' treeQuery(c("Case1", "Case2"), "CS")
#' @export
treeQuery<-function(cases, variant)
{
  ...
  #Eastern variants (No more TreeList_East table. All variants use TreeList table now.)
...
#' Build SQL Query for FVS Cases Table
#'
#' Builds and returns a string of SQL statements to read CaseID, StandID,
#' Groups, and Variant from the FVS_Cases table for a given run title. Can be
#' used with dbGetQuery from RSQLite. Added in query for Stand_CN in new verison.
#'
#' @param runTitle Character string for FVS run title.
#' @return Character string of SQL statements.
#' @examples
#' caseQuery("MyRunTitle")
#' @export
caseQuery <- function(runTitle)
{
  query<- paste("SELECT FVS_Cases.Stand_CN, FVS_Cases.StandID, FVS_Cases.CaseID, FVS_Cases.Groups,",
                "FVS_Cases.Variant",
                "FROM FVS_Cases",
...
#' Build SQL Query for FVS Compute Table
#'
#' Builds and returns a string of SQL statements to read in data from the
#' FVS_Compute table. Can be used with dbGetQuery from RSQLite.
#'
#' @param cases Character vector of case IDs.
#' @return Character string of SQL statements.
#' @examples
#' computeQuery(c("Case1", "Case2"))
#' @export
...
#' Build SQL Query for FVS PotFire Table
#'
#'@description
#' Builds and returns a string of SQL statements to read in data from the
#' FVS_Potfire or FVS_PotFire_East table depending on the variant specified. Can
#' be used with dbGetQuery from RSQLite.
#'
#' This query will read in the following fields from the FVS_PotFire table:
#'
#' "CaseID, "Year", "Surf_Flame_Sev", "Surf_Flame_Mod", "Tot_Flame_Sev",
...
#'
#' @param cases Character vector of case IDs.
#' @param variant Two-character variant code This code is used to determine what potential fire database table data should be read from (FVS_Potfire or FVS_Potfire_East). Valid variant codes: "AK", "BM", "CA", "CI", "CR", "CS", "EC", "EM",  "IE", "KT", "LS", "NC", "NE", "PN", "SN", "SO", "TT", "UT", "WC", "WS", "OC", "OP"
#' @return Character string of SQL statements.
#' @examples
#' potFireQuery(c("Case1", "Case2"), "CS")
#' @export
...
#' Get FVS_POTFIRE Table Variable Names
#'
#' Returns the variable names included in the FVS_POTFIRE database table except
#' for StandID. Used to order columns for combining with FVS_POTFIRE_EAST output
#' so it can be combined with output from the standard FVS_POTFIRE_TABLE.
#'
#' @return Character vector of FVS_POTFIRE variable names.
#' @examples
#' getPotFireVars()
#' @export
...
#' Build SQL Query for FVS Fuels Table
#'
#' Builds and returns a string of SQL statements to read in data from the
#' FVS_Fuels table. This query will read in all fields from the FVS_Fuels
#' table. The `fuelsQuery` function can be invoked outside of main.R for testing
#' purposes and can be used in the `dbGetQuery` function of the RSQLite R package.
#' @param cases Character vector of case IDs.
#' @return Character string of SQL statements.
#' @examples
#' fuelsQuery(c("Case1", "Case2"))
#' @export
...
#' Build SQL Query for FVS Carbon Table
#'
#' Builds and returns a string of SQL statements to read in data from the
#' FVS_Carbon table. This query will read in all fields from the FVS_Carbon
#' table. The `carbonQuery` function can be invoked outside of main.R for testing
#' purposes and can be used in the `dbGetQuery` function of the RSQLite R package.
#'
#' @param cases Character vector of case IDs.
#' @return Character string of SQL statements.
#' @examples
#' carbonQuery(c("Case1", "Case2"))
#' @export
...
#' Collect IDs for SQL Query
#'
#' @description
#' Takes in a character vector of IDs used for querying a database
#' and collects them into a single string where IDs are surrounded by parentheses
#' and separated by commas.
#' @description
#' Example IDs: "Stand1", "Stand2", "Stand3" would be collected into the
...
#' @return Character string of IDs surrounded by parentheses and separated by commas.
#' @examples
#' collectID(c("Stand1", "Stand2", "Stand3"))
#' @export
...
#' Get Index Names from SQLite Database
#'
#' Returns the names of indices that exist in the input SQLite database.
#'
#' @param input Connection to a SQLite database (.db)
#' @return Character vector of index names that exist in input argument.
#' @examples
#' con <- RSQLite::dbConnect(RSQLite::SQLite(), ":memory:")
#' getIndexNames(con)
#' @export
...
#' Set CaseID Indices in FVS Tables
#'
#' Creates indices on the CaseID field for all FVS database tables found in the
#' input database. Should be used if input database in `main()` (main.R) was not
#' created in the FVS interface (i.e rFVS or FVS run through command line). Some
#' Logic in this function was borrowed from mkDBIndices function
#' (fvsRunUtilities.R). This had been set to automatically run
#'
#' @param input Connection to a SQLite database (.db)
#' @return Invisibly returns 0 if successful, 1 if no tables found.
#' @examples
#'  con <- RSQLite::dbConnect(RSQLite::SQLite(), ":memory:")
#'  setCaseIndices(con)
#' @export
setCaseIndices <- function(input)
{
  ...
  for(i in 1:length(dbTables))
  {
    ...
    if(grepl('FVS_', dbTab, fixed = T) & ("CaseID" %in% dbFields))
    {
      ...
      #Create index query
      indexQuery=paste0("create index if not exists IDX_",dbTab," on FVS_",dbTab," (CaseID);")
...
#' Remove CaseID Indices from FVS Tables
#'
#' Removes all indices created by `setCaseIndices()` in FVS
#' database tables found in input database argument. Some Logic in this function
#' was borrowed from `mkDBIndices()` (fvsRunUtilities.R).
#'
#' @param input Connection to a SQLite database (.db)
#' @return Invisibly returns 0 if successful, 1 if no tables found.
#' @examples
...
#' Extract Group from FVS Group Labels
#'
#' Not in use.
#' Takes a string of FVS group labels and returns the group following the input string identifier as specified in the label argument.
#'
#' @param groups String containing group labels as formatted in the FVS_Cases table.
#' @param label Label used to extract the group of interest (e.g., "ERU=").
#' @return Extracted group string or "ALL" if not found.
#' @examples
#' getGroup("ERU=MCW,AGE=30", "ERU=")
#' @export
