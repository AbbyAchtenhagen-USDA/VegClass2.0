...
##' Calculate Dominance Group 6040
#'
#' Calculates the dominance group (DOM6040) for a stand based on species composition, proportions, and size attributes.
#' Uses USFS Region 6/8 rules for dominance classification.
#'
#' @param dTotal Numeric. Stand-level trees or basal area per acre.
#' @param spec1 Character vector of species in the stand (USDA Plants 4-digit codes).
#' @param prop1 Numeric vector of the proportions of TPA/BA by species in the stand.
#' @param diam1 Numeric vector of basal area weighted average diameters by species.
#' @param height1 Numeric vector of arithmetic or basal area weighted average heights by species.
#'
#' @return Character string with the DOM6040 code for the stand.
#' @examples
#' spec1 <- c("PIPO", "PSME", "ABGR")
#' prop1 <- c(0.5, 0.3, 0.2)
#' diam1 <- c(12, 10, 8)
#' height1 <- c(60, 55, 50)
#' computeDominance6040(100, spec1, prop1, diam1, height1)
#' @export
