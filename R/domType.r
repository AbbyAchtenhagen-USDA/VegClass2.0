...
#' Calculate Stand Dominance Type (Region 3)
#'
#' Calculates stand dominance type using USFS Region 3 rule sets in accordance
#' with Vandendriesche, D., 2013. A Compendium of NFS Regional Vegetation
#' Classification Algorithms. USDA Forest Service. Fort Collins, CO (R3-1 -
#' R3-3).
#'
#' @param data  Data frame containing tree records from a single stand or plot. Data frame must contain a column corresponding to stand/plot ID, DBH, species code (USDA plant symbol), expansion factor, and crown width for each tree record.
#' @param stand Character string corresponding to name of column pertaining to stand or plot ID associated with tree records in data argument. By default, this value is set to "StandID".
#' @param species Character string corresponding to name of column pertaining to USDA plant symbols of tree records in data argument. By default, this argument is set to "SpeciesPLANTS".
#' @param dbh Character string corresponding to name of column pertaining to DBH of tree records in data argument. By default, this argument is set to"DBH".
#' @param expf Character string corresponding to name of column pertaining to TPA of tree records in data argument. By default, this argument is set to "TPA".
#' @param crwidth Character string corresponding to name of column pertaining to crown width values of tree records in data argument. By default, this argument is set to "CrWidth".
#' @param TPA Numeric. Trees per acre for the stand/plot.
#' @param CC Numeric. Canopy cover (uncorrected for overlap).
#' @param debug Logical; if TRUE, prints debug output.
#'
#' @return Named list containing:
#'   \item{DOMTYPE}{Dominance type}
#'   \item{DCC1}{Dominant species/genus/category or species/genus/category occurring before underscore in dominance type}
#'   \item{XDCC1}{Percent canopy cover corrected for overlap represented by DCC1}
#'   \item{DCC2}{Species/genus/category occurring after underscore in dominance type}
#'   \item{XDCC2}{Percent canopy cover corrected for overlap represented by DCC2}
#' @examples
#' domTypeR3(df, TPA=100, CC=30)
#' @export
...
#' Find Mutually Exclusive Genus Index
#'
#' Used in the Region 3 dominance type algorithm (see domTypeR3) to determine a genus that is mutually exclusive from
#' the genus of the most abundant species by percent canopy cover in lead 14 of
#' R3 dominance type algorithm. This function takes the name of the most
#' abundant species, a vector with names of each genus (sorted by genera with most
#' to least canopy cover), and a named vector of genera (not sorted by percent canopy cover).
#'
#' @param sp Name of the most abundant species by percent canopy cover for current stand in `domType()`
#' @param spNames Vector of genera names ordered by abundance in terms of percent canopy cover for current stand in `domType()`
#' @param genusList Vector of genera whose indices are named by each species found in current stand in `domType()`
#'
#' @return
#' Index where the name of the most abundant genus does not match the genus in
#' the input list of genera.
#' @examples
#' excGenusSp("PIPO", c("Pinus", "Abies"), c(PIPO="Pinus", ABGR="Abies"))
...
#Chad Keyser in 2023. This R8 classification is not the official rule set for
#USFS R8. It was developed for a specific project in North Carolina. Five dominance type
#classifications are calculated in this
...
#' Calculate Stand Dominance Type (Region 8)
#'@description
#' Calculates stand dominance type using USFS Region 8 rule sets determined by
#' Chad Keyser in 2023. This R8 classification is not the official rule set for
#' USFS R8. It was developed for a specific project in North Carolina. Five dominance
#' type classifications are calculated:
#' advanced regeneration, non-merchantable trees, pulpwood sized trees, saw
#' timber sized trees, and the entire stand. Criteria for
#' determining dominance type is as follows:
#' @description
#' Advance regeneration dominance
#'
...
#'   for that size class.
#'
#' @param data Data frame of tree records for a single stand or plot. Must include a stand/plot ID column.
#' @param stand Name of the stand/plot ID column. Default is "StandID".
#' @param attrList List of size and density attributes for species, as produced by `plotAttr()`.
#' @param debug Logical; if TRUE, prints debug output.
#'
#' @return Named list containing:
#'   \item{SSDOMSPP}{Dominance type of advanced regeneration}
#'   \item{NMDOMSPP}{Dominance type of non-merchantable trees}
#'   \item{PWDOMSPP}{Dominance type of pulpwood sized trees}
#'   \item{STDOMSPP}{Dominance type of saw timber sized trees}
#'   \item{DOMTYPE}{Dominance type of stand}
#' @examples
#' attrList <- list(ALL = c(SSTPA=100, NMBA=50, PWBA=30, STBA=20, BA=200, TPA=300),
#'                PIPO = c(SSTPA=60, NMBA=30, PWBA=10, STBA=10, BA=100),
#'                ABGR = c(SSTPA=40, NMBA=20, PWBA=20, STBA=10, BA=100))
#' df <- data.frame(StandID="A", X=1)
#' domTypeR8(df, attrList=attrList)
#' @export
